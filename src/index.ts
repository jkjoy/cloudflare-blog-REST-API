...
// Legacy admin remains available while Vue pages are migrated incrementally.
app.get('/wp-admin/legacy', async (c) => {
  const siteSettings = await getSiteSettings(c.env);
    ...
    const i18n = {
      ...
      translations: {
        zh: {
          ...
          common: { add: '添加', edit: '编辑', delete: '删除', create: '创建', update: '更新', save: '保存', cancel: '取消', close: '关闭', view: '查看', upload: '上传', insert: '插入', search: '搜索', filter: '筛选', all: '全部', yes: '是', no: '否', actions: '操作', status: '状态', date: '日期', name: '名称', slug: '别名', description: '描述', count: '数量', required: '必填', optional: '可选', newAdmin: '返回新版后台' },
      ...
      app.innerHTML = \`
        ...
        <div class="main-content">
          <div class="top-bar">
            ...
            <h1>\${title}</h1>
            <div class="user-info">
              <a class="button button-secondary" href="/wp-admin">\${i18n.t('common.newAdmin')}</a>
  ...
  "moments": []
    ...
    async function init() {
      ...
      if (authenticated) {
        ...
        const resolvedRoute = routes[requestedRoute] ? requestedRoute : '/';
        await navigate(resolvedRoute);

        if (params.get('create') === '1') {
          if (resolvedRoute === '/posts') await window.showCreatePostModal();
          if (resolvedRoute === '/pages') window.showCreatePageModal();
          return;
        }

        const editId = parseInt(params.get('edit') || '', 10);
        if (Number.isInteger(editId) && editId > 0) {
          if (resolvedRoute === '/posts') await window.editPost(editId);
          if (resolvedRoute === '/pages') await window.editPage(editId);
        }
      }
    }
